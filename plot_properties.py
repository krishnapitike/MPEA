...
df = pd.read_csv("data.csv", delimiter=",")  # Specify a semicolon delimiter
data=df.to_numpy()
...
#numbers only
numbers_only = np.array([list(map(int, re.findall(r'\d+', item))) for item in data[:,0]], dtype='int')
row_sums = numbers_only.sum(axis=1, keepdims=True)  # Sum along rows
numbers_only = numbers_only / row_sums

#numbers only
# numbers_only = [list(map(int, re.findall(r'\d+', item))) for item in data[:,0]]
# Normalize each row by the sum of its elements
# numbers_only = [[x / sum(row) for x in row] if sum(row) != 0 else row for row in numbers_only]
...
for icombination, combination in enumerate(combinations):
    ...
    for isubPlot, subPlot in enumerate(subPlotList):
        axes[isubPlot].scatter(selected_frac,selected_rows[:,subPlotList[isubPlot]],facecolor='none',edgecolors=colors[icombination],marker=markers[icombination])
        ...
        axes[isubPlot].plot(unique_fracs, mean_values, c=colors[icombination], marker=markers[icombination],label=combination)
        ...
        axes[isubPlot].set_xlim([0.3,1.0])
